...
const Skills = () => {
  return (
    <section className=''>
        <div className='h-190 sm:h-140 md:h-120 lg:h-115 bg-linear-to-br from-indigo-50 via-white to-sky-100 dark:from-indigo-950 dark:via-zinc-900 dark:to-zinc-950 px-6 lg:px-30 flex items-center justify-center flex-col sm:gap-7 sm:px-10'>
            ...
            <div className='w-full grid h-auto grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 flex-1 gap-4 pb-10 lg:pb-15'>
                {skillCategories[0].skills.map(skill => {
                    ...
                    return (
                      <motion.div whileHover={{y:-5}} transition={{duration:0.2}} whileInView={{scale:1.05}} key={skill.name} className='flex items-center justify-center gap-1 flex-col bg-white dark:bg-purple-950 shadow-md rounded-xl p-4 group cursor-pointer'>
